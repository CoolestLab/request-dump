...
GLOBAL_LD_FLAGS := -X 'github.com/CoolestLab/serverless-go-template/build.Version=${GIT_DESCRIBE}' \
	-X 'github.com/CoolestLab/serverless-go-template/build.GoVersion=${GO_VERSION}' \
	-X 'github.com/CoolestLab/serverless-go-template/build.GitHash=${GIT_HASH}' \
	-X 'github.com/CoolestLab/serverless-go-template/build.BuildTime=${CURRENT_TIME}'
...
release:
	@for TARGET_OS in ${RELEASE_OS} ; do \
		for TARGET_ARCH in ${RELEASE_ARCH} ; do \
			...
			EXTRA_FLAGS="-X 'github.com/CoolestLab/serverless-go-template/build.OSArch=$${TARGET_OS}/$${TARGET_ARCH}'" ; \
